script_name("AdminHelper for Advance RP")
script_version("1") 
...
function main() 
	...
	if unload == false then sampAddChatMessage(tag .. "" .. thisScript().name .. " успешно запущен. Версия: " .. thisScript().version .. ". Разработчик скрипта: {4682B4}Anthony_Martini", 0xFFA500)  sampAddChatMessage(tag .. "Помощь по скрипту: {FFA500}/hhelp{ffffff}. Приятного пользования {ff6666}:3", 0xFFA500) if check == false then sampAddChatMessage(tag.. "{ff0000}Внимание! {ffffff}Установите нужные Вам настройки, используя {FFDAB9}/aset", 0xFFA500) end end
	...
	versialua = thisScript().version
	obnovv = obnov:read()
	if obnovv > versialua then
		sampAddChatMessage(tag.. "{ff0000}Внимание! {ffffff}Обнаружена новая версия скрипта ({FFDAB9}" ..obnovv.. "{ffffff}). Сейчас произойдет процесс автоматического обновления", 0xFFA500)
		downloadUrlToFile("https://github.com/tsurik/jadahadjddw/raw/master/AdminHelper.lua", thisScript().path, function(id, status)
			if status == dlstatus.STATUSENDDOWNLOADDATA then
				local f = io.open('moonloader\\ah\\config.ini', 'w') 
				f:write("[settings]\ncolor1=0\ncolor2=0\ngg=Приятной игры и хорошего настроения на Advance RP Silver =]\nprm=Обычный промокод - arp_s7 ; Гетто-промокод - arps_ghetto\nnet=Ник формата 'Имя_Фамилия' меняется через 'Донат' (9 руб).\njb=Оформите жалобу на forum.advance-rp.ru(С видео)\najb=Если вы не согласны с наказанием, пишите жалобу на форум.\nrp=Узнайте Role-Play путем, приятной игры :]\noftp=Пожалуйста не оффтопьте (При повторе может наказываться затычкой)\nnapom=true\nkillid=true") 
				f:close()
				sampAddChatMessage(tag.. "Скрипт обновлен и сейчас будет перезапущен...", 0xFFA500)
				thisScript():reload()
			end
		end)
