...
local tag = '{FFA500}[AdminHelper]{FFFFFF} ' 
...
function main() 
	...
	else sampAddChatMessage(tag.. "Данный скрипт предназначен для использования исключительно на серверах Advance RP", 0xffffff) unload = true thisScript():unload() end
	if unload == false then sampAddChatMessage(tag .. "" .. thisScript().name .. " успешно загружен. ID версии: {FFDAB9}" .. scrptnum .. "{ffffff}. Разработчик скрипта: {4682B4}Anthony_Martini", 0xffffff)  sampAddChatMessage(tag .. "Помощь по скрипту: {FFA500}/hhelp{ffffff}. Приятного пользования {ff6666}:3", 0xffffff) if check == false then sampAddChatMessage(tag.. "{ff0000}Внимание! {ffffff}Установите нужные Вам настройки, используя {FFDAB9}/aset", 0xffffff) end end
	...
	while true do
		...
		if isKeyDown(18) and isKeyJustPressed(50) then
			sampAddChatMessage(tag.. "Велосипед был выдан", 0xffffff)
			sampSendChat("/vec 510 " ..color1.. " " ..color2)
		end
		if isKeyDown(18) and isKeyJustPressed(51) then
			if respavn == false then
				sampAddChatMessage(tag.. "Нажмите комбинацию еще раз для подтверждения действия ({FFDAB9}Alt + 3{ffffff})", 0xffffff)
				respavn = true
			else
		...
		if isKeyDown(18) and isKeyJustPressed(48) then
			if hpall == false then
				sampAddChatMessage(tag.. "Нажмите комбинацию еще раз для подтверждения действия ({FFDAB9}Alt + 0{ffffff})", 0xffffff)
				hpall = true
			else
		...
		if napom == true then
			if os.date("%M",os.time()) == "59" and os.date("%S",os.time()) == "00" then
				sampAddChatMessage(tag.. "{ff0000}Внимание! {ffffff}Payday будет через {FFDAB9}1 минуту{ffffff}. Не выходите в AFK =]", 0xffffff)
				wait(1000)
			end 
		end
		...
		if resultMain == true then
			if buttonMain == 1 then
				if listMain == 0 then
					sampShowDialog(10, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/gg", "{FFA500}Текущий текст: {ffffff}" ..gg, "Редакт.", "Назад", 1)
				end
				if listMain == 1 then
					sampShowDialog(11, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/prm", "{FFA500}Текущий текст: {ffffff}" ..prm, "Редакт.", "Назад", 1)
				end
				if listMain == 2 then
					sampShowDialog(12, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/net", "{FFA500}Текущий текст: {ffffff}" ..net, "Редакт.", "Назад", 1)
				end
				if listMain == 3 then
					sampShowDialog(13, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/jb", "{FFA500}Текущий текст: {ffffff}" ..jb, "Редакт.", "Назад", 1)
				end
				if listMain == 4 then
					sampShowDialog(14, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/ajb", "{FFA500}Текущий текст: {ffffff}" ..ajb, "Редакт.", "Назад", 1)
				end
				if listMain == 5 then
					sampShowDialog(15, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/rp", "{FFA500}Текущий текст: {ffffff}" ..rp, "Редакт.", "Назад", 1)
				end
				if listMain == 6 then
					sampShowDialog(16, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/oftp", "{FFA500}Текущий текст: {ffffff}" ..oftp, "Редакт.", "Назад", 1)
				end
				if listMain == 7 then
					sampShowDialog(20, "{FFA500}" ..tag.. "Изменение цвета {FFA500}/a", "{FFA500}Текущий цвет: {ffffff}" ..acolor.. "\n{ff0000}[!] {ffffff}Используйте {FFA500}0x[html цвет] {ffffff}или {FFA500}off {ffffff}для выключения функции {ff0000}[!]", "Редакт.", "Назад", 1)
				end
				if listMain == 8 then
					sampShowDialog(21, "{FFA500}" ..tag.. "Изменение цвета {FFA500}/ans", "{FFA500}Текущий цвет: {ffffff}" ..anscolor.. "\n{ff0000}[!] {ffffff}Используйте {FFA500}0x[html цвет]{ffffff} или {FFA500}off {ffffff}для выключения функции {ff0000}[!]", "Редакт.", "Назад", 1)
				end
				if listMain == 9 then
					if napom == true then ini.settings.napom = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Напоминания о зарплате {ff0000}выключены", 0xffffff) 
					elseif napom == false then ini.settings.napom = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Напоминания о зарплате {00ff00}включены", 0xffffff) end
				end
				if listMain == 10 then
					if killid == true then ini.settings.killid = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "ID в kill-листе {ff0000}выключены", 0xffffff) 
					elseif killid == false then ini.settings.killid = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "ID в kill-листе {00ff00}включены", 0xffffff) end
				end
				if listMain == 11 then
					if adSF == true then ini.settings.adSF = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Перенос объявлений в SF-консоль {ff0000}выключено", 0xffffff) 
					elseif adSF == false then ini.settings.adSF = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Перенос объявлений в SF-консоль {00ff00}включено", 0xffffff) end
				end
				if listMain == 12 then
					if counter_capture == true then ini.settings.counter_capture = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Счетчик варнов для каптов {ff0000}выключен", 0xffffff) 
					elseif counter_capture == false then ini.settings.counter_capture = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Счетчик варнов для каптов {00ff00}включен", 0xffffff) end
				end
				if listMain == 13 then
		...
		if resultMain == true then
			if buttonMain == 1 then
				stznach()
				sampAddChatMessage(tag.. "Стандартные значения заданы", 0xffffff)
		...
		if resultInput == true then
			if buttonInput == 1 then
				if #inputgg == 0 then
					...
					if #inputgg < 10 then
						sampShowDialog(10, "{FFA500}" ..tag.. "Редактирование команды {FFA500}/gg", "{FFA500}Текущий текст: {ffffff}" ..gg, "Редакт.", "Назад", 1)
						sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Слишком малельное количество символов", 0xffffff)
					else
						ini.settings.gg = inputgg
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Сохранение данных прошло успешно", 0xffffff)
					end
				end
			else 
				asettings()
			...
			if buttonInput == 1 then
				if #inputacolor == 0 then
					...
					if inputacolor == "off" then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Изменение цвета /a {ff0000}выключено", 0xffffff)
					else
			...
			if buttonInput == 1 then
				if #inputanscolor == 0 then
					...
					if inputanscolor == "off" then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Изменение цвета /ans {ff0000}выключено", 0xffffff)
					else
		...
		if ini ~= nil then
			...
			killid = ini.settings.killid
			color1 = ini.settings.color1
			color2 = ini.settings.color2
			adSF = ini.settings.adSF
			counter_capture = ini.settings.counter_capture
		end
		if prm == nil then prm = "Обычный промокод - arp_s7 ; Гетто-промокод - arps_ghetto" end
		if gg == nil then gg = "Приятной игры и хорошего настроения на Advance RP Silver =]" end
		if net == nil then net = "Ник формата 'Имя_Фамилия' меняется через 'Донат' (9 руб)." end
		if jb == nil then jb = "Оформите жалобу на forum.advance-rp.ru (с видео)" end
		if ajb == nil then ajb = "Если вы не согласны с наказанием, пишите жалобу на форум." end
		if rp == nil then rp = "Узнайте Role-Play путем, приятной игры :]" end
		if oftp == nil then oftp = "Пожалуйста не оффтопьте (При повторе может наказываться затычкой)" end
		if acolor == nil then acolor = "off" end
		if anscolor == nil then anscolor = "0xDAA520" end
		if napom == nil then napom = true end
		if killid == nil then killid = true end
...
function asettings()
	if napom == true then vnapom = "{00FF00}Включено"
	elseif napom == false then vnapom = "{ff0000}Выключено" end
	if killid == true then vkillid = "{00ff00}Включено"
	elseif killid == false then vkillid = "{ff0000}Выключено" end
	if adSF == true then vadSF = "{00ff00}Включено"
	elseif adSF == false then vadSF = "{ff0000}Выключено" end
	if counter_capture == true then vcounter_capture = "{00ff00}Включено"
	elseif counter_capture == false then vcounter_capture = "{ff0000}Выключено" end
	if acolor == "off" then vacolor = "{ff0000}Выключено"
	else vacolor = acolor end
	if anscolor == "off" then vanscolor = "{ff0000}Выключено"
	else vanscolor = anscolor end
	sampShowDialog(0, "{FFA500}" ..tag.. "Настройки скрипта", "{FFA500}Функция\t{ffffff}Текущее значение\n{FFA500}/gg\t{ffffff}" ..gg.. "\n{FFA500}/prm\t{ffffff}" ..prm.. "\n{FFA500}/net\t{ffffff}" ..net.. "\n{FFA500}/jb\t{ffffff}" ..jb.."\n{FFA500}/ajb\t{ffffff}" ..ajb.. "\n{FFA500}/rp\t{ffffff}" ..rp.. "\n{FFA500}/oftp\t{ffffff}" ..oftp.. "\n{FFA500}Цвет /a\t{ffffff}" ..vacolor.. "\n{FFA500}Цвет /ans\t{ffffff}" ..vanscolor.. "\n{FFA500}Напоминание о ЗП \t" ..vnapom.. "\n{FFA500}ID в kill-листе\t" ..vkillid.. "\n{FFA500}/ad в SF-консоль\t" ..vadSF.. "\n{FFA500}Счетчик варнов\t" ..vcounter_capture.. "\n{008000}Вернуть стандартные значения", "Ок", "Отмена", 5)
end
	...
	if #arg == 0 then
		...
		if type(arg) ~= "number" then
			...
			if arg < 400 or arg > 611 then
				sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}ID транспорта находится в пределе от 400 до 611", 0xffffff)
			else
				sampSendChat("/vec " ..arg.. " " ..color1.. " " ..color2)
				sampAddChatMessage(tag.. "Транспорт создан: {FFDAB9}ID " ..arg, 0xffffff)
			end
		end
	end
end
...
function capturewarns()
	if captnak == true then
		...
		if rifawarns == 0 and grovewarns == 0 and ballaswarns == 0 and aztecwarns == 0 and vagoswarns == 0 then
			grove = "{FFDAB9}варнов еще не было выдано"
			ballas = ""
			vagos = ""
			rifa = ""
			aztec = ""
		end
		sampAddChatMessage(tag.. "Статистика варнов: " ..grove.. "" ..ballas.. "" ..aztec.. "" ..vagos.. "" ..rifa, 0xffffff)
	else
		sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}В данный момент не проходит захват территории", 0xffffff)
	end
end
...
function statwarns()
	if statwarns == true then
		...
		sampSendChat("/msg 1 Статистика предупреждений: " ..grove.. "" ..ballas.. "" ..aztec.. "" ..vagos.. "" ..rifa)
		statwarns = false
	end
end
...
function ttime(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /tt [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /tt [id]", 0xffffff)
			...
			if arg < 0 or arg > 999 then
				...
				if sampIsPlayerConnected(arg) then
					...
					sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Игрока с таким ID нет на сервере", 0xffffff)
				end
			end
		end
	end
...
function quit()
	sampAddChatMessage(tag.. "Работа скрипта была завершена", 0xffffff)
	thisScript():unload()
end

function cinformation(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /cinf [id/название авто (с маленькой)]", 0xffffff)
	else
		checkcar()
		inform = infa[arg]
		if inform ~= nil then
			sampAddChatMessage(tag.. "" ..inform, 0xffffff)
		else
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Данных об этом транспорте не обнаружено", 0xffffff)
		end
	end
end
...
function vcolor(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /vcol [id цвета 1] [id цвета 2]", 0xffffff)
	else
		...
		if type(color1) ~= "number" then
			...
			if type(color2) ~= "number" then
				sampAddChatMessage(tag.. "Используйте /vcol Используйте /vcol [id цвета 1] [id цвета 2]", 0xffffff)
			else
				if color1 < 0 or color1 > 255 then
					sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Диапазон ID цветов от 0 до 255", 0xffffff)
				else
					if color2 < 0 or color2 > 255 then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "ID цветов сохранено: " ..color1.. ", " ..color2, 0xffffff)
					end
				end
			end
		end
...
function offtop(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /oftp [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /oftp [id]", 0xffffff)
...
function goodgame(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /gg [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /gg [id]", 0xffffff)
...
function arecon(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /re [id игрока] [id нарушителя]", 0xffffff)
	else
					...
					if idd < 0 or idd > 999 then
						...
						if sampIsPlayerConnected(id) then
							if sampIsPlayerConnected(idd) then
								...
								sampAddChatMessage(tag.. "Для завершения наблюдения используйте {ff0000}/reoff{ffffff}. Если игрок получит наказание {ff0000}/pn", 0xffffff)
							...
							sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Игрока с данным ID нет на сервере", 0xffffff)
						end
					end
				end
			end
...
function nonrpnicknames()
	if nrpnicks == "" then
		sampShowDialog(22820, tag.. "Запросы на смену ника", "{ffffff}Очередь на смену ника {ff0000}пустая", "Закрыть")
	else
		sampShowDialog(22821, tag.. "Запросы на смену ника", nrpnicks, "Закрыть")
	end
end
...
function punish(arg)
	if #arg == 0 then
		if recon then
			...
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Вы не начинали слежку за игроком. Можете использовать {FFDAB9}/pn [id]", 0xffffff)
		end
	end
	arg = tonumber(arg)
	if type(arg) == "number" then
...
function areconoff(arg)
	if #arg == 0 then
		if recon then
			...
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Вы не начинали слежку за игроком. Можете использовать {FFDAB9}/reoff [id]", 0xffffff)
		end
	end
	if arg == "-1" then
		sampAddChatMessage(tag.. "Данные о слежке обнулены", 0xffffff)
		recon = false
	else
		arg = tonumber(arg)
		if type(arg) == "number" then
...
function rrp(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /rp [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /rp [id]", 0xffffff)
...
function ajaloba(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /ajb [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /ajb [id]", 0xffffff)
...
function promokod(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /prm [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /prm [id]", 0xffffff)
...
--	захватить территорию -10092289
--	-1717986817
	if string.find(color, "1721329407", 1, true) and string.find(text, "[Смена non-rp имени]", 1, true) then
		nrpnicks = nrpnicks.. "{FFA500}[" ..os.date("%X",os.time()).. "] {ffffff}" ..text.. "\n"
	end
	if adSF == false then
		if string.find(color, "13369599", 1, true) and string.find(text, "Отправил", 1, true) then
			print(text)
			return false
		end
		if string.find(color, "10027263", 1, true) and string.find(text, "Объявление проверил сотрудник СМИ", 1, true) then
			print(text)
			return false
		end
	end
	if counter_capture == true then
		if string.find(color, "-10092289", 1, true) and string.find(text, "инициировал захват", 1, true) then
			sampAddChatMessage(tag.. "Счетчик наказаний {00ff00}включен", 0xffffff)
			grovewarns = 0
			ballaswarns = 0
			aztecwarns = 0
			rifawarns = 0
		...
		if string.find(color, "-11521793", 1, true) and string.find(text, "предупреждение", 1, true) and captnak == true then
			if string.find(text, "ballas", 1, true) or string.find(text, "Ballas", 1, true) or string.find(text, "баллас", 1, true) or string.find(text, "Баллас", 1, true) then
				ballaswarns = ballaswarns+1
			elseif string.find(text, "grove", 1, true) or string.find(text, "Grove", 1, true) or string.find(text, "грув", 1, true) or string.find(text, "Грув", 1, true) then
				grovewarns = grovewarns+1
			elseif string.find(text, "rifa", 1, true) or string.find(text, "Rifa", 1, true) or string.find(text, "рифа", 1, true) or string.find(text, "Рифа", 1, true) then
				rifawarns = rifawarns+1
			elseif string.find(text, "vagos", 1, true) or string.find(text, "Vagos", 1, true) or string.find(text, "вагос", 1, true) or string.find(text, "Вагос", 1, true) then
				vagoswarns = vagoswarns+1
			elseif string.find(text, "aztec", 1, true) or string.find(text, "Aztec", 1, true) or string.find(text, "ацтек", 1, true) or string.find(text, "Ацтек", 1, true) then
				aztecwarns = aztecwarns+1
			end
		end
		if captnak == true and string.find(color, "-10092289", 1, true) and string.find(text, "захватить территорию", 1, true) or string.find(text, "захватили территорию", 1, true) then
			statwarns = true
			if grovewarns > 0 then
				grove = "{009900}| Grove: " ..grovewarns.. " |"
			else
			...
			if rifawarns == 0 and grovewarns == 0 and ballaswarns == 0 and aztecwarns == 0 and vagoswarns == 0 then
				grove = "{FFDAB9}варнов на продолжении капта не было выдано"
				ballas = ""
				vagos = ""
				rifa = ""
				aztec = ""
			...
			sampAddChatMessage(tag.. "Статистика варнов: " ..grove.. "" ..ballas.. "" ..aztec.. "" ..vagos.. "" ..rifa, 0xffffff)
			if statwarns == true then
				sampAddChatMessage(tag.. "Для того, чтобы огласить статистику варнов в общий чат, используйте {FFDAB9}/wst", 0xffffff)
			end
			captnak = false
		end
...
function net(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /net [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /net [id]", 0xffffff)
...
function slap(arg)
if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /slap [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /slap [id]", 0xffffff)
...
function jaloba(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /jb [id]", 0xffffff)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /jb [id]", 0xffffff)
...
function thread_function(option)
	...
	if option == "update" then
		...
		if obnov == nil then 
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Произошел сбой подключения", 0xffffff)
		else
			obnovv = obnov:read()
			...
			if obnovv > scrptnum then
				...
				sampAddChatMessage(tag.. "Скрипт успешно обновлен до версии {FFDAB9}" ..obnovv.. "{FFFFFF}!", 0xffffff)
				obnov:close()
				return
			else
				sampAddChatMessage(tag.. "Обновлений не обнаружено. Данная версия является {00ff00}актуальной", 0xffffff)
			end
		end
	end
end
...
function update()
	...
	sampAddChatMessage(tag.. "Устанавливается соединение с сервером...", 0xffffff)
	thread:run("update")
end
