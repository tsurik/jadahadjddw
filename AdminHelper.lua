...
function main() 
	...
	if unload == false then sampAddChatMessage(tag .. "" .. thisScript().name .. " успешно загружен. Версия: " .. thisScript().version .. ". Разработчик скрипта: {4682B4}Anthony_Martini", 0xFFA500)  sampAddChatMessage(tag .. "Помощь по скрипту: {FFA500}/hhelp{ffffff}. Приятного пользования {ff6666}:3", 0xFFA500) if check == false then sampAddChatMessage(tag.. "{ff0000}Внимание! {ffffff}Установите нужные Вам настройки, используя {FFDAB9}/aset", 0xFFA500) end end
