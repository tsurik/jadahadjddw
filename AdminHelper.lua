	...
	while true do
		...
		if resultMain == true then
			if buttonMain == 1 then
				...
				if listMain == 10 then
					if killid == true then ini.settings.killid = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "ID в kill-листе {ff0000}выключены", 0xFFA500) 
					elseif killid == false then ini.settings.killid = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "ID в kill-листе {00ff00}включены", 0xFFA500) end
				end
				if listMain == 11 then
...
function capturewarns()
	if captnak == true then
		...
		sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}В данный момент не проходит захват территории", 0xFFA500)
	end
end
