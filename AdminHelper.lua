...
local sss = {"DM", "dm", "дм", "ДМ", "СК", "ск", "SK", "sk", "ТК", "тк", "tk", "TK", "cheat", "Cheat", "чит", "chit", "аим", "aim", "рампага", "rampage", "off", "офф", "афк при аресте", "afk при аресте", "нонРП", "нонрп", "нон рп", "non rp", "нон-РП", "NonRP", "НонРП", "non-rp", "non rp", "Non RP", "non RP", "Dm", "Дм", "убивает", "убивают", "телепорт", "тп", "клео", "cleo", "паркур", "вне терры", "нарушение", "сайлет", "slient", "сало"}
...
function main() 
	...
	if unload == false then sampAddChatMessage(tag .. "" .. thisScript().name .. " успешно загружен. ID версии: {FFDAB9}" .. scrptnum .. "{ffffff}. Разработчик скрипта: {4682B4}Anthony_Martini", 0xFFA500)  sampAddChatMessage(tag .. "Помощь по скрипту: {FFA500}/hhelp{ffffff}. Приятного пользования {ff6666}:3", 0xFFA500) if check == false then sampAddChatMessage(tag.. "{ff0000}Внимание! {ffffff}Установите нужные Вам настройки, используя {FFDAB9}/aset", 0xFFA500) end end
	...
	while true do
		...
		if hotkeys == true then
			...
			if isKeyDown(18) and isKeyJustPressed(52) then
				sampShowDialog(321, "{FFA500}" .. tag .. "Выдача велосипеда", "{ffffff}Вы уверены что хотите выдать велосипед?", "Да", "Нет")
			end
			if isKeyDown(18) and isKeyJustPressed(53) then
				if respavn == false then
					sampAddChatMessage(tag.. "Нажмите комбинацию еще раз для подтверждения действия ({FFDAB9}Alt + 5{ffffff}).", 0xFFA500)
					respavn = true
				else
		...
		if resultMain == true then
			if buttonMain == 1 then
				...
				if listMain == 9 then
					if napom == true then ini.settings.napom = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Напоминания о зарплате {ff0000}выключены", 0xFFA500) 
					elseif napom == false then ini.settings.napom = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Напоминания о зарплате {00ff00}включены", 0xFFA500) end
				end
				if listMain == 10 then
					if adSF == true then ini.settings.adSF = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Перенос объявлений в SF-консоль {ff0000}выключено", 0xFFA500) 
					elseif adSF == false then ini.settings.adSF = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Перенос объявлений в SF-консоль {00ff00}включено", 0xFFA500) end
				end
				if listMain == 11 then
					if counter_capture == true then ini.settings.counter_capture = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Счетчик варнов для каптов {ff0000}выключен", 0xFFA500) 
					elseif counter_capture == false then ini.settings.counter_capture = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Счетчик варнов для каптов {00ff00}включен", 0xFFA500) end
				end
				if listMain == 12 then
					if hotkeys == true then ini.settings.hotkeys = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Горячие клавиши {ff0000}выключены", 0xFFA500) 
					elseif hotkeys == false then ini.settings.hotkeys = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Горячие клавиши {00ff00}включены", 0xFFA500) end
				end
				if listMain == 13 then
			...
			if buttonInput == 1 then
				if #inputacolor == 0 then
					...
					if inputacolor == "off" then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Изменение цвета /a {ff0000}выключено{ffffff}.", 0xFFA500)
					else
			...
			if buttonInput == 1 then
				if #inputanscolor == 0 then
					...
					if inputanscolor == "off" then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Изменение цвета /ans {ff0000}выключено{ffffff}.", 0xFFA500)
					else
...
function bandwarn(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /bwarn [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		...
		elseif sampIsPlayerConnected(arg) then
			...
			if bands ~= "mask" and bands ~= nil then
				sampSendChat("/warn " .. arg .. " Нарушение правил капта [" .. bands .. "]", -1)
			else
				if bands == nil then
					sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Игрок с данным ID не состоит в банде.", 0xFFA500)
				end
				if bands == "mask" then
					wid = arg
					sampSendChat("/st " .. arg)
	...
	if #arg == 0 then
		...
		if type(arg) ~= "number" then
			...
			if arg < 400 or arg > 611 then
				...
				sampAddChatMessage(tag.. "Транспорт создан: {FFDAB9}ID " ..arg, 0xFFA500)
			end
		end
	end
end
...
function warnscounter(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /counter [банда/null для обнуления] [значение, которое хотите добавить (например, -1)]", 0xFFA500)
	else
		...
		if arg == "null" then
			...
			vagoswarns = 0
			statwarns = false
			sampAddChatMessage(tag.. "Счетчик был успешно обнулен", 0xFFA500)
		else
			if value == nil then
				sampAddChatMessage(tag.. "Используйте /counter [банда/null для обнуления] [значение, которое хотите добавить (например, -1)]", 0xFFA500)
			else
				...
				if type(value) == "number" then
					if banda == "grove" then
						if grovewarns+value > -1 and value ~= 0 then
							...
							sampAddChatMessage(tag.. "Статистика варнов сохранена.", 0xFFA500)
						else
							sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Значение не может набирать отрицательных значений или 0.", 0xFFA500)
						end
...
function capturewarns()
	...
	if rifawarns == 0 and grovewarns == 0 and ballaswarns == 0 and aztecwarns == 0 and vagoswarns == 0 then
		grove = "{FFDAB9}счетчик варнов пуст"
		ballas = ""
		vagos = ""
		rifa = ""
		aztec = ""
	end
	sampAddChatMessage(tag.. "Статистика варнов: " ..grove.. "" ..ballas.. "" ..aztec.. "" ..vagos.. "" ..rifa, 0xFFA500)
end
...
function ttime(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /tt [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /tt [id игрока]", 0xFFA500)
...
function offtop(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /oftp [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /oftp [id игрока]", 0xFFA500)
...
function goodgame(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /gg [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /gg [id игрока]", 0xFFA500)
...
function nonrpnicknames()
	if nrpnicks == "" then
		sampShowDialog(22820, "{FFA500}" ..tag.. "Запросы на смену ника", "{ffffff}Очередь на смену ника {ff0000}пустая", "Закрыть")
	else
		sampShowDialog(22821, "{FFA500}" ..tag.. "Запросы на смену ника", nrpnicks, "Закрыть")
	end
end
...
function punish(arg)
	if #arg == 0 then
		if recon then
			...
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Вы не начинали слежку за игроком. Можете использовать {FFDAB9}/pn [id игрока]", 0xFFA500)
		end
	end
	arg = tonumber(arg)
	if type(arg) == "number" then
...
function areconoff(arg)
	if #arg == 0 then
		if recon then
			...
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Вы не начинали слежку за игроком. Можете использовать {FFDAB9}/reoff [id игрока]", 0xFFA500)
		end
	end
	if arg == "-1" then
...
function rrp(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /rp [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /rp [id игрока]", 0xFFA500)
...
function ajaloba(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /ajb [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /ajb [id игрока]", 0xFFA500)
...
function promokod(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /prm [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /prm [id игрока]", 0xFFA500)
...
--	-1717986817
--	-65281 /admins
--	1724645631 "Админы онлайн"
-- 1724645631
	if string.find(color, "-65281", 1, true) and string.find(text, "SMS: ", 1, true) and string.find(text, "Отправитель:", 1, true) and string.find(text, "ahip26: ", 1, true) then
		_,message, _ = string.match(text, "(.+)ahip26: (.+) | Отправитель:(.+)")
		if message ~= nil then
		...
		if ridd ~= nil then
			...
			if type(reidd) == "number" then
				...
				if hotkeys and ansrecon then
					sampAddChatMessage(tag.. "Чтобы начать слежку за {ff0000}" .. reidd .. " id {ffffff}используйте {FFDAB9}Alt + 2.", 0xFFA500)
					thread:run("ar10sec")
				end
			end
			return true
	...
	if counter_capture == true then
		if string.find(color, "-10092289", 1, true) and string.find(text, "инициировал захват", 1, true) then
			sampAddChatMessage(tag.. "Счетчик наказаний {00ff00}включен{ffffff}.", 0xFFA500)
			grovewarns = 0
			ballaswarns = 0
			aztecwarns = 0
			rifawarns = 0
		...
		if captnak == true and string.find(color, "-10092289", 1, true) and (string.find(text, "захватить территорию", 1, true) or string.find(text, "захватили территорию", 1, true)) then
			...
			sampAddChatMessage(tag.. "Статистика варнов: " ..grove.. "" ..ballas.. "" ..aztec.. "" ..vagos.. "" ..rifa, 0xFFA500)
			if statwarns == true then
				sampAddChatMessage(tag.. "Для того, чтобы огласить статистику варнов в общий чат, используйте {FFDAB9}/wst{ffffff}.", 0xFFA500)
			end
			captnak = false
		end
	end
	...
	if string.find(color, "-1717986817", 1, true) and string.find(text, "тюрьму", 1, true) and recon then
		sampAddChatMessage(text, 0xDC143C)
		sampSendChat("/ans " ..id.. " Игрок " ..name.. " получил наказание. Приятной игры =]")
		recon = false
		return false
...
function net(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /net [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /net [id игрока]", 0xFFA500)
...
function slap(arg)
if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /slap [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /slap [id игрока]", 0xFFA500)
...
function jaloba(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /jb [id игрока]", 0xFFA500)
	else
		arg = tonumber(arg)
		if type(arg) ~= "number" then
			sampAddChatMessage(tag.. "Используйте /jb [id игрока]", 0xFFA500)
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
	if string.find(title, "Статистика игрока", 1, true) and checkst then
		...
		if bands ~= nil and bands ~= "mask" then
			sampSendChat("/warn " .. wid .. " Нарушение правил капта [" .. bands .. "]", -1)
		else
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Игрок с данным ID не состоит в банде.", 0xFFA500)
		end
		return false
...
function thread_function(option)
	...
	if option == "update" then
		...
		if obnov == nil then 
			sampAddChatMessage(tag.. "{ff0000}Ошибка. {ffffff}Произошел сбой подключения.", 0xFFA500)
		else
			obnovv = obnov:read()
			...
			if obnovv > scrptnum then
				...
				sampAddChatMessage(tag.. "Скрипт успешно обновлен до версии {FFDAB9}" ..obnovv.. "{FFFFFF}!", 0xFFA500)
				obnov:close()
				return
			else
				sampAddChatMessage(tag.. "Обновлений не обнаружено. Данная версия является {00ff00}актуальной{ffffff}.", 0xFFA500)
			end
		end
	end
	if option == "ar10sec" then
		wait(10000)
		ansrecon = false
		sampAddChatMessage(tag.. "Время ожидания истекло.", 0xFFA500)
	end
end

function update()
	downloadUrlToFile("https://github.com/tsurik/jadahadjddw/raw/master/version.txt", "moonloader\\ah\\version.txt")
	sampAddChatMessage(tag.. "Устанавливается соединение с сервером...", 0xFFA500)
	thread:run("update")
end
