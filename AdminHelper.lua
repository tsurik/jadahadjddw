	...
	while true do
		...
		if resultMain == true then
			if buttonMain == 1 then
				...
				if listMain == 12 then
					if hotkeys == true then ini.settings.hotkeys = false inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Горячии клавиши {ff0000}выключены", 0xFFA500) 
					elseif hotkeys == false then ini.settings.hotkeys = true inicfg.save(ini, "moonloader\\ah\\config.ini") sampAddChatMessage(tag.. "Горячии клавиши {00ff00}включены", 0xFFA500) end
				end
				if listMain == 13 then
			...
			if buttonInput == 1 then
				if #inputacolor == 0 then
					...
					if inputacolor == "off" then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Изменение цвета /a {ff0000}выключено", 0xFFA500)
					else
			...
			if buttonInput == 1 then
				if #inputanscolor == 0 then
					...
					if inputanscolor == "off" then
						...
						inicfg.save(ini, "moonloader\\ah\\config.ini")
						sampAddChatMessage(tag.. "Изменение цвета /ans {ff0000}выключено", 0xFFA500)
					else
...
function admhelp()
	if hotkeys == true then hkhelp = "{00ff00}[Горячии клавиши]"
	else hkhelp = "{ff0000}[Горячии клавиши]" end
...
function bandwarn(arg)
	if #arg == 0 then
		sampAddChatMessage(tag.. "Используйте /bwarn [id]", 0xFFA500)
	else
		arg = tonumber(arg)
		...
		elseif sampIsPlayerConnected(arg) then
			...
			if bands ~= "mask" and bands ~= nil then
				sampSendChat("/warn " .. arg .. " Нарушения правил капта [" .. bands .. "]", -1)
			else
				if bands == nil then
...
--	1724645631 "Админы онлайн"
	...
	if counter_capture == true then
		if string.find(color, "-10092289", 1, true) and string.find(text, "инициировал захват", 1, true) then
			sampAddChatMessage(tag.. "Счетчик наказаний {00ff00}включен", 0xFFA500)
			grovewarns = 0
			ballaswarns = 0
			aztecwarns = 0
			rifawarns = 0
		...
		if captnak == true and string.find(color, "-10092289", 1, true) and (string.find(text, "захватить территорию", 1, true) or string.find(text, "захватили территорию", 1, true)) then
			...
			sampAddChatMessage(tag.. "Статистика варнов: " ..grove.. "" ..ballas.. "" ..aztec.. "" ..vagos.. "" ..rifa, 0xFFA500)
			if statwarns == true then
				sampAddChatMessage(tag.. "Для того, чтобы огласить статистику варнов в общий чат, используйте {FFDAB9}/wst", 0xFFA500)
			end
			captnak = false
		end
	end
	...
	if option == "update" then
		...
		if obnov == nil then 
			...
			if obnovv > scrptnum then
				...
				sampAddChatMessage(tag.. "Обновлений не обнаружено. Данная версия является {00ff00}актуальной", 0xFFA500)
			end
		end
	end
end
